...
class VideoCamera(object):
    ...
    def get_all_encodings(self):
        ...
        print(self.enterd_emp)
        print(self.ap_emp)
        for x in self.enterd_emp:
            self.ap_emp.remove(x['name'])
        print(self.ap_emp)
    ...
    def get_frame(self):
        ...
        for self.face_encoding, self.face_location in zip(self.face_encodings, self.face_locations):
            ...
            if self.name in self.emp_name:
                if self.name in self.ap_emp:
                    self.ap_emp.remove(self.name)
                    self.current_fream_time = datetime.now().strftime("%H:%M:%S")
                    self.current_fream_date = datetime.now().strftime("%d/%m/%Y")
                    self.enter_entry = data(date=self.current_fream_date, name=self.name, enter_time=self.current_fream_time, exit_time='0')
                    self.enter_entry.save()
